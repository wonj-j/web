...
<body>
			...
			<div class="row">
				...
				<div class="col-md-12 col-lg-4">
					<nav aria-label="breadcrumb"
						...
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a href="index.html">${area.area}</a></li>
							<li class="breadcrumb-item"><a href="property-grid.html">${area.city}</a>
							</li>
							<li class="breadcrumb-item" aria-current="page"><a
								class="breadcrumb-item active" href="property-grid.html">${area.category}</a>
							</li>
						</ol>
					</nav>
				</div>
		...
		<div class="container">
            ...
            <div class="tab-pane fade" id="pills-plans" role="tabpanel" aria-labelledby="pills-plans-tab">
              ...
              <div class="col-md-10 offset-md-1 col-lg-10 offset-lg-1">
                <!--  -->
                <div class="title-box-d">
                  <h3 class="title-d section-t4" id="commentcnt"></h3>
                </div>

				<div style="text-align:right; margin-right:30px; padding-bottom: 10px;">
				</div>
	                <div class="box-comments" id="cm">
                  ...
                  <div id="reviewstar">
                    <div id="review-info">
                      ...
                      <div class="starRev">
                        ...
                        <div class="starR2 star space" data-star="8"></div>
                        <div class="starR3 star space" data-star="9"></div>
                    
                        <span class="point" id="point">0.0<span class="man">/5.0 </span>
                    ...
                    <div id="myreview">
                      <textarea name="" id="comment" cols="84" rows="6" placeholder="리뷰를 남겨주세요
                                      ." required></textarea>
	...
	$("#leave-review").click(function(){
		...
		/* 유저 번호를 가져와서 id를 보내야함 */
		/* var id = $("#id").val(); */
		var point = $("#point").text();
		var myimg = $("#myimg").attr("src");
		var comment = $("#comment").val();

 		var data = "&point="+point+"&myimg="+myimg+"&comment="+comment+"&seq="+${seq}; 
 		
		$.ajax({
			type: "POST",
			url: "/bnb/tour/comment.do",
			data: data,
			dataType: "xml",
			success: function(result){
					...
					var dates = $(item).find("dates").text();
					var depth = $(item).find("depth").text();
					
					
					
					var newPanel = String.format("<li><div class='comment-avatar'><img src='{0}' alt=''></div><div class='comment-details'><h4 class='comment-author'>{1}</h4><span>{2}</span><div class='wrap-revstar'><div class='revstar'></div> <b class='spoint'>{3}</b></div><p class='comment-description'>{4}</p><form method='POST' action='/bnb/tour/tourcomment.do'><input type='button' value='reply' class='reply' /><input type='text' class='reply-text' style='display: none;'/></form></div></li>"
								, img
								, id
								, dates
								, point
	...
	var end;
	 ...
	 $("#pills-plans-tab").click(function(){
	 ...
	 function view() {
		 $.ajax({
				type: "POST",
				url: "/bnb/tour/commentlist.do",
				data: "seq="+${seq}+"&begin="+begin+"&end="+end,
				dataType: "xml",
				success: function(result){
					...
					$(result).find("item").each(function(index, item){
						...
						var Panel = String.format("<ul class='list-comments' id='posts'><li><div class='comment-avatar'><img src='{0}' alt=''></div><div class='comment-details'><h4 class='comment-author'>{1}</h4><span>{2}</span><div class='wrap-revstar'><div class='revstar'></div> <b class='spoint'>{3}</b></div><p class='comment-description'>{4}</p><form method='POST' action='/bnb/tour/tourcomment.do'><input type='button' value='reply' class='reply' /><input type='text' class='reply-text' style='display: none;'/></form></div></li></ul>"
						, img
						, id
						, dates
						, point
					...
					pagebar = $(result).find("pagebar").html();
					$("#paging").append(pagebar);
					var cnt = $(result).find("totalcount").text();
					$("#commentcnt").text(cnt);
					//pagebar만들기
					
					$("table tr td ul li a").off("click");
					page();
